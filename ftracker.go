...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runCalories := ((runningCaloriesMeanSpeedMultiplier * runSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkSpeed := meanSpeed(action, duration)
	speed := math.Pow(walkSpeed*kmhInMsec, 2)
	walkCalories := ((walkingCaloriesWeightMultiplier*weight + (speed/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimCalories := (swimSpeed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
	return swimCalories
}
